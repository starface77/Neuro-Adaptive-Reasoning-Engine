...
from ..execution.sandbox import safe_load_module, SecurityError
...
def discover_rule(
    ...
    # 3. Evaluate on holdout + perturbations
    scores = []
    ...
    if best_score < 0.6:  # Threshold for generalization
...
def sample_candidate_rules(episodes: List[Dict], n: int = 5) -> List[Dict]:
    ...
    prompt = """You are a rule discovery system. Given examples of (query, solution) pairs, propose MULTIPLE candidate transformation rules.
...
"""
    ...
    try:
        ...
        # Parse multiple rules from response
...
def evaluate_operator(
    ...
    for ep in holdout_eps:
        ...
        try:
            if not trigger_fn(query):
                continue  # Rule doesn't claim to handle this
            ...
            holdout_total += 1  # Count as failure
            logging.debug(f"[Library Learning] Execution error: {e}")

    holdout_acc = holdout_correct / holdout_total if holdout_total > 0 else 0.0

    # 2. Perturbation robustness (weight: 0.2)
    # Generate simple perturbations of holdout queries
    perturb_correct = 0
    perturb_total = 0

    for ep in holdout_eps[:2]:  # Test on 2 examples
        ...
        for perturbed_query in perturbations:
            try:
                ...
                # For perturbations, we expect similar structure
                # (This is heuristic - ideally need oracle for perturbed queries)
    ...
    # 3. No false positives on train (weight: 0.1)
    # Rule should trigger on train examples
    train_trigger = 0
    for ep in train_eps:
        try:
