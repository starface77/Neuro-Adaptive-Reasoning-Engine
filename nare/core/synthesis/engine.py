...
def _rebuild_prompt_with_files(base_prompt: str, file_context: dict, query: str) -> str:
    """Rebuild prompt with current file context.

    BUG FIX #5: Instead of concatenating files to the prompt on each iteration,
    rebuild the entire prompt with all requested files. This prevents context explosion.
    """
    prompt = base_prompt
    ...
    prompt += f"\n\nNow fix the bug using the provided files. Output in the required format.\n"
...
def verified_synthesis(
    ...
    base_prompt = query  # BUG FIX #5: Store original prompt
    prompt = query

    _fetched_files: set = set()
    _file_context: dict = {}  # BUG FIX #5: Store file contents separately
        ...
        if needed and file_provider:
            if needed not in _fetched_files:
                ...
                if file_content is not None:
                    log.info(
                        "[VS] Agent requested file '%s' — adding to context",
                        needed,
                    )
                    _file_context[needed] = file_content[:15000]
                else:
                    log.warning(
                        "[VS] Agent requested file '%s' but it was not found",
                        needed,
                    )
                    _file_context[needed] = "[FILE NOT FOUND]"

                prompt = _rebuild_prompt_with_files(base_prompt, _file_context, query)
                continue
            else:
