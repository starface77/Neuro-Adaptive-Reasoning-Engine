...
def _rebuild_prompt_with_files(base_prompt: str, file_context: dict, query: str) -> str:
    ...
    Rebuilds the entire prompt with all requested files to prevent context explosion
    from concatenating files on each iteration.
    """
    prompt = base_prompt
...
def verified_synthesis(
    ...
    _file_context: dict = {}  # Store file contents for synthesis
