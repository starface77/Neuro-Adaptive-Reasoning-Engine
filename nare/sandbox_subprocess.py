...
from .sandbox import SecurityError, validate_code

logger = logging.getLogger(__name__)


# Hard defaults; callers can override per-call.
DEFAULT_TIMEOUT_SECONDS: float = 5.0
DEFAULT_MEM_LIMIT_BYTES: int = 256 * 1024 * 1024  # 256 MiB
DEFAULT_CPU_LIMIT_SECONDS: int = 5
DEFAULT_FSIZE_LIMIT_BYTES: int = 0  # zero-byte file writes
DEFAULT_NOFILE_LIMIT: int = 32


# A small Python program that imports nothing dangerous, reads a JSON
# request from stdin, runs the skill, and writes a JSON response to
# stdout. Embedded as a string so we never depend on a shippable
# package layout in the child process.
_CHILD_RUNNER = r"""
import json
import sys
...
def _make_preexec(
    ...
    import resource  # POSIX-only; imported lazily so Windows still imports the module
    ...
    def _apply():
        ...
        try:
            ...
            pass
        # CPU seconds: process is killed with SIGXCPU on overrun.
        try:
            resource.setrlimit(
                resource.RLIMIT_CPU,
            ...
            pass
        # Disable file writes entirely (skill must communicate via stdout).
        try:
            resource.setrlimit(
...
def safe_execute_subprocess(
    ...
    *,
    ...
    """
    # Defense in depth: same AST validation that the in-process path uses.
    ...
    # Empty env so $HOME / $PATH / secrets don't leak. We still need
    # a minimal PATH for python startup on some distros, but the child
    # never spawns subprocesses so an empty PATH is fine.
