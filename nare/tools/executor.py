...
def parse_tool_calls(text: str) -> List[Dict[str, Any]]:
    ...
    # First, try XML format: <create_file>filepath\ncontent</create_file>
    ...
    for match in xml_matches:
        ...
        if tool_name == "create_file":
            ...
            # Try to extract path and content from nested tags
            ...
            if path_match and content_match:
                ...
                if len(lines) >= 2:
                    ...
                    # Remove any XML tags from filepath
            ...
            if len(lines) >= 3:
                ...
                tool_calls.append({
                    'tool': tool_name,
                    'args': [filepath, lines[1].strip(), lines[2].strip()]
                })
    ...
    # If XML format found, return those
    if tool_calls:
        return tool_calls

    # Fallback: Function call format
    # Pattern to match function calls with better handling of multiline strings
    ...
    for match in matches:
        ...
        try:
            import ast
            # We construct a valid Python expression to parse
            ...
            if isinstance(tree.body[0], ast.Expr) and isinstance(tree.body[0].value, ast.Call):
                ...
                for arg in call.args:
                    if isinstance(arg, ast.Constant):
                        ...
                        # If it's not a constant string, try to evaluate it
        ...
        # Fallback to simple split if ast.parse fails (e.g. malformed python)
        args = []
...
def execute_tool_call(tool_name: str, args: List[str], stream_callback: Optional[Callable] = None, working_dir: str = ".") -> Optional[str]:
    ...
    try:
        if tool_name == "create_file":
            ...
            # Only resolve relative paths - keep absolute paths as-is
            if not os.path.isabs(filepath):
                filepath = os.path.join(working_dir, filepath)

            # Notify stream callback about file writing start
            if stream_callback:
            ...
            # Notify stream callback about file editing start
            if stream_callback:
                stream_callback('start', filepath, None)

            # Edit file with streaming
