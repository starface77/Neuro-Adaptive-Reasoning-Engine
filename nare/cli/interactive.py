...
def ask_compile_skill(query: str) -> bool:
    """Ask if user wants to compile this execution as a reusable skill.

    Args:
        query: The query that was just executed successfully

    Returns:
        True if user wants to compile, False otherwise
    """
    # Filter out trivial queries
    query_lower = query.lower().strip()

    # Don't ask for greetings or single words
    if len(query_lower) < 10:
        return False

    # Don't ask for simple questions
    trivial_patterns = ['привет', 'ку', 'что', 'как', 'hello', 'hi', 'а', 'да', 'нет']
    if any(query_lower.startswith(p) for p in trivial_patterns) and len(query_lower) < 20:
        return False

    console = Console()
    console.print()
    console.print(f"  [#D77757]Compile this as a reusable skill?[/]")
    console.print(f"  [#666666]Query: {query[:60]}...[/]" if len(query) > 60 else f"  [#666666]Query: {query}[/]")
    console.print()

    choices = [
        ("yes", "Yes, compile as skill"),
        ("no", "No, skip")
    ]
