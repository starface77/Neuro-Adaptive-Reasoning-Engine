...
class NareSession:
    ...
    async def solve_agentic(self, query: str, thinking_display=None, resume_state: Optional[Dict] = None) -> dict:
        ...
        if not resume_state:
            ...
            if route_result["route_decision"] in ["FAST", "REFLEX", "COMPILED_SKILL", "DIRECT"]:
        ...
        if run.final_answer:
            ...
            if run.stop_reason in clean_success_reasons:
                try:
                    ...
                    # Ask user if they want to compile this as a skill (only for meaningful EDIT tasks)
    ...
    async def _solve_async(self, query: str, thinking_display=None, resume_state: Optional[Dict] = None) -> dict:
        ...
        # Ask to compile after successful Router execution (only for meaningful EDIT tasks, not conversational)
