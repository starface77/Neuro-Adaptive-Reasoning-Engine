...
def read_file(path: str, working_dir: Optional[str] = None) -> ToolResult:
...
# ─────────────────────────────────────────────────────────────────────
# Git status (read-only convenience)
# ─────────────────────────────────────────────────────────────────────
...
def build_default_registry(working_dir: Optional[str] = None) -> ToolRegistry:
    ...
    def _wd(fn):
        # Bind working_dir into each builtin without changing its signature
    ...
    reg.register(Tool(
        name="update_todos",
        description=(
            "Replace the visible task list. Use at the start of a multi-step "
            "task to declare the plan, and after each completed step to mark "
            "progress. The UI renders a checklist panel."
        ),
        parameters=[
            ToolParam(
                "items",
